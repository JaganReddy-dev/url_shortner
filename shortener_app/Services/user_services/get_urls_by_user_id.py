...
from shortener_app.Models.v1.request.get_urls_by_user_id import GetUrlsByIdRequest
from shortener_app.Models.v1.response.get_urls_by_user_id import GetUrlsByUserIdResponse


async def get_urls_by_user_id(user: GetUrlsByIdRequest) -> GetUrlsByUserIdResponse:
    """
    ...
    cursor = url_map_collection.find(
        {"user_id": user.user_id}, {"long_url": 1, "short_url_path": 1, "_id": 0}
    )
    async for document in cursor:
        url_mappings.append(
            {"long_url": document["long_url"], "short_url": document["short_url_path"]}
        )
